...
class Parser:
    ...
    def list_expr(self):
        ...
        #para List vacia, q solo contenga "[]"
    ...
    #como ya tenemos multilinea, vamos a tener diferentes formas para el IF
    def if_expr(self):
        res = ParseResult()
    ...
    #se modifico el while para que pueda utilizar varias lineas
    def while_expr(self):
        res = ParseResult()
...
#asignacion de valores para null, false y true
...
#funcion base para la funcion integrada    
...
#Clase funcion, heredando la funcion base    
class Function(BaseFunction):
    ...
    def execute(self, args):
        res = RTResult()
        semantico = Semantico()
        exec_ctx = self.generate_new_context() #agregando funciones de la clase base
        
        #agregando funciones de la clase base
        res.register(self.check_and_populate_args(self.arg_names, args, exec_ctx))
        if res.error: return res
            
        value = res.register(semantico.visita(self.body_node, exec_ctx))
        if res.error: return res
...
#Clase para la contruccion de una funcion   
class BuiltInFunction(BaseFunction):
    ...
    def execute(self, args):
        ...
        method = getattr(self, method_name, self.no_visita_method)
    ...
    def no_visita_method(self, node, context):
...
#creacion de constantes para cada funcion
...
class Semantico:
    ...
    #para cuando visitamos un nodo List
    def visita_NodoList(self, node, contexto):
        res = RTResult()
        elements = []
        
        for element_nodes in node.element_nodes:
            elements.append(res.register(self.visita(element_nodes, contexto)))
            if res.error: return res
        
        return res.success(
            List(elements).set_context(contexto).set_pos(node.pos_start, node.pos_end)
        )
    ...
    def visita_IfNode(self, node, context):
        ...
        for condition, expr, should_return_null in node.cases:
            condition_value = res.register(self.visita(condition, context))
            if res.error: return res
            
            if condition_value.is_true():
                expr_value = res.register(self.visita(expr, context))
                if res.error: return res
    ...
    def visita_ForNode(self, node, context):
        ...
        start_value = res.register(self.visita(node.start_value_node, context))
        if res.error: return res
        
        end_value = res.register(self.visita(node.end_value_node, context))
        if res.error: return res
        
        if node.step_value_node:
            step_value = res.register(self.visita(node.step_value_node, context))
            if res.error: return res
        else:
            step_value = Number(1)
        ...
        while condition():
            ...
            elements.append(res.register(self.visita(node.body_node, context)))
            if res.error: return res
    ...
    def visita_WhileNode(self, node, context):
        ...
        while True:
            condition = res.register(self.visita(node.condition_node, context))
            if res.error: return res
    ...
    def visita_CallNode(self, node, context):
        ...
        for arg_node in node.arg_nodes:
            args.append(res.register(self.visita(arg_node, context)))
            if res.error: return res
